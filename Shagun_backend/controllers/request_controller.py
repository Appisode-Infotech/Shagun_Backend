...
def request_callback(request_obj):
    try:
        with connection.cursor() as cursor:
            sql_query = """ INSERT INTO user_callback_request (uid,type,event_date,event_type,status, created_on,)
             VALUES (%s,%s,%s,%s,%s,%s) """
...
def update_callback_request(callback_obj):
    try:
        with connection.cursor() as cursor:
            sql_query = """ UPDATE  user_callback_request SET status = %s, completed_by = %s WHERE id = % s  """
